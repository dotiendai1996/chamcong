...
namespace Hinnova.Organizations
{
    ...
    public class OrganizationUnitAppService : HinnovaAppServiceBase, IOrganizationUnitAppService
    {
        ...
        public async Task<List<string>> GetAllUnit()
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                var result = await conn.QueryAsync<string>(sql: "SELECT * FROM dbo.AbpOrganizationUnits WHERE IsDeleted = 0");
